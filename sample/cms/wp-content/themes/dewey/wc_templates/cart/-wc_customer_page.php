	...
	<?php if( have_posts() ) : usces_remove_filter(); ?>
		...
		<?php if( usces_is_membersystem_state() ) : ?>
			<?php if( !welcart_basic_have_ex_order() ) : ?>
				<h5><?php _e('The member please enter at here.','usces'); ?> <strong>※ご利用には会員登録が必ず必要です。</strong></h5>
			<?php endif; ?>
				...
				<form action="<?php usces_url('cart'); ?>" method="post" name="customer_loginform" onKeyDown="if(event.keyCode == 13){return false;}">
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="customer_form">
						...
						<tr>
							...
							<td><input name="loginpass" id="mailaddress1" type="password" value="" />
		...
		<?php if( !welcart_basic_have_ex_order() ) : ?>
				<h5><?php _e('The nonmember please enter at here.','usces'); ?> <strong>※ご利用には会員登録が必ず必要です。</strong></h5>
				...
				<form action="<?php echo USCES_CART_URL; ?>" method="post" name="customer_form" onKeyDown="if(event.keyCode == 13){return false;}">
					...
					<table border="0" cellpadding="0" cellspacing="0" class="customer_form">
						...
						<tr>
							...
							<td colspan="2"><input name="customer[mailaddress1]" id="mailaddress1" type="text" value="<?php esc_attr_e($usces_entries['customer']['mailaddress1']); ?>" style="ime-mode: inactive" placeholder="PCメールが受信可能なメールアドレスをご利用ください。" /></td>
						</tr>
						<tr>
							<th scope="row"><em><?php _e('*', 'usces'); ?></em><?php _e('e-mail adress', 'usces'); ?>(<?php _e('Re-input', 'usces'); ?>)</th>
							<td colspan="2"><input name="customer[mailaddress2]" id="mailaddress2" type="text" value="<?php esc_attr_e($usces_entries['customer']['mailaddress2']); ?>" style="ime-mode: inactive" placeholder="確認のため、再入力ください。" /></td>
						</tr>
						<?php if( usces_is_membersystem_state() ) : ?>
						<tr>
							<th scope="row"><?php if( $member_regmode == 'editmemberfromcart' ) : ?><em><?php _e('*', 'usces'); ?></em><?php endif; ?><?php _e('password', 'usces'); ?></th>
							<td colspan="2"><input name="customer[password1]" style="width:120px" type="password" value="<?php esc_attr_e($usces_entries['customer']['password1']); ?>" />お好きなパスワード。(半角英数6文字以上)</td>
						</tr>
						<tr>
							<th scope="row"><?php if( $member_regmode == 'editmemberfromcart' ) : ?><em><?php _e('*', 'usces'); ?></em><?php endif; ?><?php _e('Password (confirm)', 'usces'); ?></th>
							<td colspan="2"><input name="customer[password2]" style="width:120px" type="password" value="<?php esc_attr_e($usces_entries['customer']['password2']); ?>" />確認のため、再入力ください。</td>
						</tr>
						<?php endif; ?>
					</table>
