...
if ($cnt && !$submit){
	...
	if ($sdata_line){
		foreach ($sdata_line as $key => $value){
			//文字列分割
			list($count,$plan,$plan_limit,$account,$store,$customer_id,$customer,$degree,$zip,$pref,$addr,$addr2,$phone,$mobile,$email,$memo,$firsttime,$lasttime,$flag,$dsp,$cancel_flag,$c_date,$c_reason1,$c_reason2,$c_reason3,$c_reason4,$c_reason5,$c_reasonO,$c_reasonOf) = split("&&",$value);
			if ($count == $cnt){
				$check = "true";
				break;
			}
...
///////////////////////////////////
function write_data(){
	global $plan,$plan_limit,$account,$store,$customer_id,$customer,$degree,$zip,$pref,$addr,$addr2,$phone,$mobile,$email,$memo,$firsttime,$lasttime,$flag,$dsp,$cancel_flag,$c_date,$c_reason1,$c_reason2,$c_reason3,$c_reason4,$c_reason5,$c_reasonO,$c_reasonOf;
	//カウンタ読み込み
	...
	$sdata_line = read_data();
	//一行化
	$new_line = $count."&&".$plan."&&".$plan_limit."&&".$account."&&".$store."&&".$customer_id."&&".$customer."&&".$degree."&&".$zip."&&".$pref."&&".$addr."&&".$addr2."&&".$phone."&&".$mobile."&&".$email."&&".$memo."&&".$firsttime."&&".$lasttime."&&".$flag."&&".$dsp."&&".$cancel_flag."&&".$c_date."&&".$c_reason1."&&".$c_reason2."&&".$c_reason3."&&".$c_reason4."&&".$c_reason5."&&".$c_reasonO."&&".$c_reasonOf."\n";
	if ($sdata_line){
...
//-------------------------------------------------------↓
print <<< END_OF_HTML
    <td><input type="number" name="plan_limit" value="$plan_limit" class="form-tel form-control" placeholder="数字のみ" maxlength="13" style="ime-mode: disabled;"></td>
	</tr>
<tr>
...
print <<< END_OF_HTML
	...
	<tr>
		<th>郵便番号 <div class="infotip"><abbr title="数字7桁だけを入力すれば、以下の都道府県や住所を自動入力します。番地やビル名などは入力ください。" rel="tooltip">?</abbr></div></th><td><input type="number" id="zip" name="zip" value="$zip" class="form-zip form-control" placeholder="(数字のみ) 自動変換" maxlength="8" style="ime-mode: disabled;"></td>
	</tr><tr>
		<th>都道府県</th><td>
END_OF_HTML;
//-------------------------------------------------------↑
