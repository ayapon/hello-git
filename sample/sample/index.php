...
//		if (extension_loaded("mbstring")){$word = mb_convert_kana($word,"aKCV");$word = strtoupper($word);}
		...
		foreach ($line as $value){
			list($count,$customer,$kana,$company,$section,$section2,$degree,$zip,$pref,$addr,$addr2,$phone,$mobile,$memo,$birthyear,$birthmonth,$birthday,$firsttime,$lasttime,$pic,$no_dm,$lastvisit,$dsp,$bottle_0,$bottle_num_0,$bottle_quant_0,$bottle_1,$bottle_num_1,$bottle_quant_1,$bottle_2,$bottle_num_2,$bottle_quant_2,$bottle_3,$bottle_num_3,$bottle_quant_3,$bottle_4,$bottle_num_4,$bottle_quant_4,$otherbottle,$otherbottle_num,$otherbottle_quant,$friends_0,$friends_1,$friends_2,$friends_3,$friends_4,$friends_5,$friends_6,$friends_7,$friends_8,$friends_9) = explode("&&",$value);
			$str = $company.$phone.$memo.$pic.$bottle_0.$bottle_1.$bottle_2.$bottle_3.$bottle_4.$otherbottle.$friends_0.$friends_1.$friends_2.$friends_3.$friends_4.$friends_5.$friends_6.$friends_7.$friends_8.$friends_9;
// 			if (extension_loaded("mbstring")){
// 				$str = mb_convert_kana($str,"aKCV");
// 				$str = strtoupper($str);
// 			}
...
function html_linkmain($line_link){
	...
	global $start,$end,$direct,$company_logs,$addr_logs,$birthday_logs,$pic_logs,$phone_logs,$mobile_logs,$lastvisit_logs,$friends_logs;
	//ＵＲＬの設定
	$pageurl = $this_file."?";
...
//-------------------------------------------------------↓
print <<< END_OF_HTML
      <input name="searchname" type="text" class="form-searchword form-control" value="$searchname" size="20" placeholder="お名前">
...
if ($category || $searchname || $searchword){
		echo "	</div><div class='form-group search_result'><strong>検索結果</strong>　"."\n";
	$result_txt = "";
...
if ($line_link){
	...
	$company = fix_text($company);
		...
		echo "<td><strong><a href='details.php?cnt=$count'>".$string."</a></strong>"."\n";
	...
	//今月の誕生日にマーク
		...
		echo "<br /><span class='x-small'>".$string."</span></td>"."\n";


		//ボトル
		echo "<td class='bottle_tbl'>"."\n";
	if($bottle_0 <> '') {
		echo "<a href='details.php?cnt=$count'>".$bottle_0." "."\n";
		if($bottle_num_0 <> '') {
			echo " [".$bottle_num_0."] "."\n";
		}
		if($bottle_quant_0 <> '') {
	...
	if($bottle_1 <> '') {
		...
		echo "<a href='details.php?cnt=$count'>".$bottle_1." "."\n";
		if($bottle_num_1 <> '') {
			echo " [".$bottle_num_1."] "."\n";
		}
		if($bottle_quant_1 <> '') {
	...
	if($bottle_2 <> '') {
		...
		echo "<a href='details.php?cnt=$count'>".$bottle_2." "."\n";
		if($bottle_num_2 <> '') {
			echo " [".$bottle_num_0."] "."\n";
		}
		if($bottle_quant_2 <> '') {
	...
	if($bottle_3 <> '') {
		...
		echo "<a href='details.php?cnt=$count'>".$bottle_3." "."\n";
		if($bottle_num_3 <> '') {
			echo " [".$bottle_num_3."] "."\n";
		}
		if($bottle_quant_3 <> '') {
	...
	if($bottle_4 <> '') {
		...
		echo "<a href='details.php?cnt=$count'>".$bottle_4." "."\n";
		if($bottle_num_4 <> '') {
			echo " [".$bottle_num_4."] "."\n";
		}
		if($bottle_quant_4 <> '') {
	...
	if($otherbottle <> '') {
		...
		echo " <a href='details.php?cnt=$count'>".$otherbottle." "."\n";
		if($otherbottle_num <> '') {
