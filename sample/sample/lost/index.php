...
$html_title = "ユーザーID・パスワード上書き変更丨顧客・ボトル管理システム";  //ページタイトル
...
if ($new_id || $new_pass1 || $new_pass2){
	if(!$new_id){    html_pass("！ユーザーIDが入力されていません"); }
	if(!$new_pass1){ html_pass("！パスワードが入力されていません"); }
	if(!$new_pass2){ html_pass("！確認パスワードが入力されていません"); }
	
	if(!preg_match("/^[!-~]+.{2,20}$/",$new_id)){ html_pass("！ユーザーIDは半角英数4〜20文字で入力してください"); }
	if(!preg_match("/^[!-~]+.{3,20}$/",$new_pass1)){ html_pass("！パスワードは半角英数4〜20文字で入力してください"); }
	
	if($new_id == $new_pass1){   html_pass("！ユーザーIDとパスワードは違う文字を入力してください"); }
	...
	<table class="table table-striped table-hover">
              ...
              <tr>
                <td><h4 class="errormsg">管理者パスワードの変更を完了しました。</h4>
                  <br>ユーザーIDとパスワードは紛失しないよう保管してください。</td>
              </tr>
            <tr>
              <td><a href="../index.php" class="btn btn-primary">顧客・ボトル管理トップ</a></td>
            </tr>
	</table>
...
function check(){
	...
	//パターンマッチ
	if(!document.form.new_id.value.match(/^[!-~]+.{2,20}$/)){
		window.alert("ユーザー名は半角文字4〜20文字で入力してください");
		return false;
	}
	if(!document.form.new_pass1.value.match(/^[!-~]+.{3,20}$/)){
		window.alert("パスワードは半角文字4〜20文字で入力してください");
		return false;
	}
	//確認パスワードチェック
...
<h3  class="acenter">ユーザーID・パスワード上書き変更</h3>
<div class="well bs-component admin_list">
ユーザーIDとパスワードを上書きして新たに登録します。
		<table>\n
END_OF_HTML;
//-------------------------------------------------------↑
if ($message){
...
print <<< END_OF_HTML
		<form name="form" method="post" action="$this_file" onSubmit="return check()" class="form-horizontal">
			...
			<label for="inputUser" class="col-lg-4 control-label">新規ユーザーID</label>
			<div class="col-lg-12">
				<input name="new_id" type="text" value="$new_id" maxlength="12" class="form-control" placeholder="半角英数4〜12文字">
			</div>
			<div class="clear"></div>
			</div>
			...
			<div class="col-lg-12">
				<input name="new_pass1" type="password" value="$new_pass1" maxlength="12" class="form-control" placeholder="半角英数4〜12文字">
			</div>
			<div class="clear"></div>
			</div>
			...
			<p>※入力したユーザーIDとパスワードは忘れないよう必ずお控えください。</p>
			
<div class="footer_navi">
            <!--<a href="../admin.php" class="btn btn-default">&lt; メニューに戻る</a>-->
			<div class="edit_btn"><input type="submit" name="Submit" value="上書き登録" class="btn btn-warning"></div>
			<div class="clear"></div>
</div>
		  <!--</form>-->
</div>
