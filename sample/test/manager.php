...
$this_file = "manager.php";

$mode = @$_GET["mode"];
// $url = $_SERVER['SCRIPT_NAME'];
//define ("file_category","config/category.dat");
define ("file_custom","custom.dat");
define ("file_link","data/data.dat");
...
//--------------------------------------------------------------------------------------管理者 [ カテゴリー ]
$this_file = $this_file."?mode=category";
//ＰＯＳＴ
...
$firstdsp_dsp  = @$_POST["firstdsp_dsp"];
$firstdsp_rank = @$_POST["firstdsp_rank"];
//
...
// 				$keyword03 = ereg_replace($radio,$category,$keyword03);
			  ...
			  write_data(file_link,$new_links);
			}
		}else{
			if ($word_list){
				foreach ($word_list as $value){
...
if ($submit2){
	//ユーザーファイルの読み込み
	$line_custom = read_custom();
	$line_custom['bannersize'] = $bannersize;
	//数値チェック
	if (is_numeric($wsize)){$line_custom['wsize'] = floor($wsize);}
	//ユーザーファイル書き換え
	write_custom($line_custom);
}
//初期表示設定
if ($submit3){
	//ユーザーファイルの読み込み
	$line_custom = read_custom();
	$line_custom['firstdsp_dsp'] = $firstdsp_dsp;
	$line_custom['firstdsp_rank'] = $firstdsp_rank;
	//ユーザーファイル書き換え
	write_custom($line_custom);
}
//ユーザーファイルの読み込み
...
// ＨＴＭＬヘッダ                //
///////////////////////////////////
function html_manager_header($html_title,$mode){
if ($mode == "regist"){$txt01 = "リンク登録";}else{$txt01 = "<a href='manager.php' class='a01'>リンク登録</a>";}
if ($mode == "category"){$txt02 = "カテゴリー、他設定";}else{$txt02 = "<a href='manager.php?mode=category' class='a01'>カテゴリー、他設定</a>";}
//-------------------------------------------------------↓
//ヘッダー1
html_header1();
print <<< END_OF_HTML
...
print <<< END_OF_HTML
  ...
  <table width="500" border="0" cellspacing="1" class="table01">
    <tr>
      <td class="td05">$txt01</td>
      <td class="td05">$txt02</td>
      <td class="td05"><a href="index.php" class="a01">公開ページ</a></td>
    </tr>
  </table>
  <br>
END_OF_HTML;
...
// 	global $cookie_time,$password,$submit_pass,$pass;
// 	//管理パスワード入力
// 	if ($submit_pass){
// 		//クッキーセット
// 		setcookie("ckpass","$pass",time()+$cookie_time * 24 * 60 * 60);
// 		$_SESSION["sspass"] = $pass;
// 	}
// 	if (!$submit_pass && !$pass){$pass = @$_SESSION["sspass"];}
// 	if ($pass == $password){
// 		if (!isset($_SESSION["sspass"])){$_SESSION["sspass"] = $pass;}
// 	}else{
// 		echo "<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"></head>";
// 		echo "<body><a href='index.php'>＜＜管理認証エラー</a></body></html>";
// 		exit();
// 	}
// }
////////////////////////////////////////////////////
...
function html_pagelink($dspcount){
	global $this_file,$pageurl,$page,$all,$amari;
...
function html_manager(){
	global $pageurl,$this_file,$category,$searchword,$word_list,$line_link,$dspcount;
	global $submit,$page,$fix,$delete_sw,$radio,$start,$end,$message,$message_error;
	global $site_name,$site_url,$fname,$keyword01,$keyword02,$keyword03,$comment;
...
//-------------------------------------------------------↓
print <<< END_OF_HTML
<table border="0" cellpadding="0" cellspacing="0" class="table-search01">
 <form name="form" method="post" action="$this_file">
  <tr>
    <td class="td-search01">検索：
...
print <<< END_OF_HTML
      ...
      <input name="searchword" type="text" class="form-searchword" value="$searchword"  size="50">
    <input name="Submit_search" type="submit" class="botton-serach" value="検索">
END_OF_HTML;
//-------------------------------------------------------↑
if ($category || $searchword){
	echo "	<input name='reset' type='submit' class='botton-serach' value='リセット'>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
  </td>
...
//-------------------------------------------------------↑
if ($category || $searchword){
	echo "<table border='0' cellpadding='0' cellspacing='0' class='table-search02'>"."\n";
	echo "  <tr>"."\n";
	$result_txt = "";
	if ($category){$result_txt .= "カテゴリー：[ <b style='color: #000000;background-color: #FFFF00;'>".$category."</b> ] ";}
	if ($searchword){$result_txt .= " 検索語：[ <b style='color: #000000;background-color: #FFFF00;'>".$searchword."</b> ] ";}
	if ($line_link){$recnt = count($line_link);}else{$recnt=0;}
	$result_txt .= " の検索結果 <b>".$recnt."</b> 件";
	echo "    <td class='td-result'>".$result_txt."</td>"."\n";
	echo "  </tr>"."\n";
	echo "</table>"."\n";
}else{
	echo "<br>\n";
...
$set_url = $this_file;
if ($page){$set_url = $pageurl."page=".$page."&";}else{$set_url = $pageurl;}
//-------------------------------------------------------↓
print <<< END_OF_HTML

    <h3 style="color:red;text-align:center;">$message $message_error</h3>

 <form name="form" enctype="multipart/form-data" method="post" action="$set_url">
...
//-------------------------------------------------------↑
// 		if ($fname){
// 			echo "<div class='up_img' style='width:100;height:auto;'><a href='".$fname."'><img src='".$fname."' style='width:100px;height:auto;'></a></div>"."\n";
// 		}else{
// 			echo "&nbsp;";
// 		}
// 	}else{
		if ($fname){
			echo "<div class='up_img' style='width:100px;height:auto;'><a href='".$fname."'><img src='".$fname."' style='width:100px;height:auto;'></a></div>"."\n";
...
//-------------------------------------------------------↓
print <<< END_OF_HTML
<table width="300" border="0" cellspacing="1" class="table01">
  <form name="form" method="post" action="$set_url">
  <tr>
    <td colspan="3" class="td-bottom">
      <input type="submit" class="button01" name="shift_up" value="▲">
	  <input type="submit" class="button01" name="shift_down" value="▼">
      <input type="submit" class="button01" name="fix" value="編集">
      <input type="submit" class="button01" name="delete_conf" value="削除">
	</td>
    </tr>
END_OF_HTML;
//-------------------------------------------------------↑
...
for($i=$start; $i<=$end; $i++){
	...
	echo "  <tr>"."\n";
	echo "    <td class='td-banner'>";
	if ($fname){
		echo "    <a href='".$fname."' target='_blank'><img src='".$fname."' style='width: 80px; height: auto;'></a>";
	}else{
		echo "&nbsp;";
	}
	echo "    </td>"."\n";
	echo "    <td class='td03'><a href='".$site_url."' target='_blank'>".$site_name."</a></td>"."\n";
	echo "    <td class='td-radio'>"."\n";
	if ($radio == $count){
		echo "      <input type='radio' name='radio' value='".$count."' checked>"."\n";
	}else{
		echo "      <input type='radio' name='radio' value='".$count."'>"."\n";
	}
	echo "    </td>"."\n";
	echo "  </tr>"."\n";
}
	...
	echo "  <tr>"."\n";
	echo "    <td colspan='3' class='td03'><div align='center'>登録データはありません</div></td>";
	echo "  </tr>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
...
////////////////////////////////////////////////////
// リンク集 [ cutlink ]　カテゴリー登録           //
////////////////////////////////////////////////////
//--------------------------------------------------------------------------------------関数定義
///////////////////////////////////
// 削除                          //
///////////////////////////////////
function delete_data_category($radio){
	global $word_list;
	$new_line = "";
	foreach ($word_list as $value){
		if ($value == $radio){
...
// 入替え                        //
///////////////////////////////////
function shift_data_category($shift_up,$shift_down){
	global $word_list,$radio;
	foreach ($word_list as $key => $value){
		if ($value == $radio){
			if($shift_up){
				if (@$word_list[($key-1)]){
					$temp_line = $word_list[$key]; 
					$word_list[$key] = $word_list[($key-1)];
					$word_list[($key-1)] = $temp_line;
				}			 
			}else{
				if (@$word_list[($key+1)]){
					$temp_line = $word_list[$key]; 
...
function html_category(){
	global $url,$line_custom,$delete_conf,$radio,$line_link,$fix,$word_list;
	html_manager_header("カテゴリー、他設定","category");
//-------------------------------------------------------↓
print <<< END_OF_HTML
<table width="500" border="0" cellspacing="1" class="table01">
  <tr>
  ...
  <form name="form2" method="post" action="$url">
  <tr>
    <td class="td01">ページ当りの表示件数</td>
    <td class="td02"><input name="dspsu" type="text" size="10" maxlength="3" value="$line_custom[dspsu]">
    件/ページ (半角数字)   </td>
    <td class="td04">
      <input type="submit" name="Submit1" class="button01" value="登録">
    </td>
  </tr>
  </form>
</table>
...
<table width="500" border="0" cellspacing="1" class="table01">
   <tr>
    <td colspan="4" class="td-top">&nbsp;</td>
  </tr>
  <form name="form1" method="post" action="$url">
  <tr>
    <td rowspan="2" class="td01">バナー表示サイズ</td>
    <td class="td02">
END_OF_HTML;
//-------------------------------------------------------↑
if ($line_custom['bannersize'] == "original"){
	echo "	<input type='radio' name='bannersize' value='original' checked>"."\n";
}else{
	echo "	<input type='radio' name='bannersize' value='original'>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
      オリジナルサイズで表示</td>
    <td rowspan="2" class="td04">
      <input type="submit" name="Submit2" class="button01" value="登録">
    </td>
  </tr>
  <tr>
    <td class="td02">
END_OF_HTML;
//-------------------------------------------------------↑
if ($line_custom['bannersize'] == "fixed"){
	echo "	<input type='radio' name='bannersize' value='fixed' checked>"."\n";
}else{
	echo "	<input type='radio' name='bannersize' value='fixed'>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
      幅サイズ指定
        <input name="wsize" type="text" size="10" maxlength="3" value="$line_custom[wsize]">
      pixel（半角数字）以下で表示</td>
  </tr>
  </form>
</table>
...
<table width="500" border="0" cellspacing="1" class="table01">
  ...
  <form name="form" method="post" action="$url">
    <tr>
      <td rowspan="2" class="td01">初期表示</td>
      <td class="td02">バナー、紹介文を\n
END_OF_HTML;
//-------------------------------------------------------↑
if(@$line_custom['firstdsp_dsp'] == "on"){
	echo "        <input type='radio' name='firstdsp_dsp' value='on' checked><b>表示する</b>"."\n";
	echo "        <input type='radio' name='firstdsp_dsp' value='off'><b>表示しない</b>"."\n";
}else{
	echo "        <input type='radio' name='firstdsp_dsp' value='on'><b>表示する</b>"."\n";
	echo "        <input type='radio' name='firstdsp_dsp' value='off' checked><b>表示しない</b>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
	  </td>
      <td rowspan="2" class="td04">
        <input type="submit" name="Submit3" class="button01" value="登録">
      </td>
    </tr>
    <tr>
      <td class="td02">ランキングで\n
END_OF_HTML;
//-------------------------------------------------------↑
if(@$line_custom['firstdsp_rank'] == "on"){
	echo "        <input type='radio' name='firstdsp_rank' value='on' checked><b>表示する</b>"."\n";
	echo "        <input type='radio' name='firstdsp_rank' value='off'><b>表示しない</b>"."\n";
}else{
	echo "        <input type='radio' name='firstdsp_rank' value='on'><b>表示する</b>"."\n";
	echo "        <input type='radio' name='firstdsp_rank' value='off' checked><b>表示しない</b>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
	  </td>
...
if ($delete_conf && $radio){
	if ($line_link){
		foreach ($line_link as $value){
			list(,,,,$keyword01,$keyword02,$keyword03,) = explode("&&",$value);
			if (($radio == $keyword01)||($radio == $keyword02)||($radio == $keyword03)){$cntsw = 1;break;}
		}
	}
	if(get_magic_quotes_gpc()){echo stripslashes($radio);}else{echo $radio;}
	echo "	</td>"."\n";
	if (!@$cntsw){
//-------------------------------------------------------↓
print <<< END_OF_HTML
    <td class="td04"><input type="submit" name="cancel" class="button01" value="キャンセル"></td>
    <td nowrap class="td04">
      <input type="submit" name="delete" class="button02" value="削除する">
	  <input type="hidden" name="radio" value="$radio">
END_OF_HTML;
//-------------------------------------------------------↑
	}else{
		echo "    <td class='td02'><font color='#FF0000'><b>！登録データのあるカテゴリーは削除できません。</b></font></td>"."\n";
		echo "    <td class='td04'><input type='submit' name='cancel' class='button01' value='キャンセル'></td>"."\n";
	}
}else{
	if ($fix && $radio){
		$radio2 = htmlspecialchars ($radio);
		if(get_magic_quotes_gpc()){$radio2 = stripslashes($radio2);}
//-------------------------------------------------------↓
print <<< END_OF_HTML
	<input name="category" type="text" size="50" value="$radio2">
	</td>
    <td class="td04"><input type="submit" name="cancel" class="button01" value="キャンセル"></td>
    <td nowrap class="td04">
      <input type="submit" name="Submit4" class="button01" value="編集登録">
	  <input type="hidden" name="radio" value="$radio">
...
print <<< END_OF_HTML
    ...
    <td class="td04">&nbsp;</td>
    <td nowrap class="td04">
      <input type="submit" name="Submit4" class="button01" value="新規登録">
END_OF_HTML;
//-------------------------------------------------------↑
	}
}
...
print <<< END_OF_HTML
    ...
    <tr>
      <td colspan="4" class="td02">&nbsp;</td>
    </tr>
  </form>
</table>
<table>
  <form name="form4" method="post" action="$url">
  <tr>
    <td colspan="4" class="td-bottom">
	<input type="submit" class="button01" name="shift_up" value="▲">
    <input type="submit" class="button01" name="shift_down" value="▼">
    <input type="submit" class="button01" name="fix" value="編集">
    <input type="submit" class="button01" name="delete_conf" value="削除">
...
if ($word_list){
	...
	$cnt = 0;
	if ($line_link){
		foreach ($line_link as $value2){
			list(,,,,$keyword01,$keyword02,$keyword03,) = explode("&&",$value2);
			if (($value == $keyword01)||($value == $keyword02)||($value == $keyword03)){$cnt++;}
		}
	}
	echo "  <tr>"."\n";
	echo "    <td class='td03'>".$value."</td>"."\n";
	echo "    <td class='td03'><div align='center'>".$cnt." 件</div></td>"."\n";
	echo "    <td class='td-radio'>"."\n";
	if ($radio == $value){
		echo "      <input type='radio' name='radio' value='".$value."' checked>"."\n";
	}else{
		echo "      <input type='radio' name='radio' value='".$value."'>"."\n";
	}
	echo "    </td>"."\n";
	if ($cnt){
		echo "    <td class='td-radio'>&nbsp;</td>"."\n";
	}else{
		echo "    <td class='td-radio'>削除可</td>"."\n";
	}
	echo "  </tr>"."\n";	
	}
}else{
	echo "  <tr>"."\n";
	echo "    <td colspan='4' class='td03'><div align='center'>登録データはありません</div></td>";
	echo "  </tr>"."\n";
}
//-------------------------------------------------------↓
print <<< END_OF_HTML
  <tr>
    <td colspan="4" class="td-bottom">&nbsp;</td>
  </tr>
  </form>
</table>
</div>
