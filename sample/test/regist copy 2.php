...
//画像リサイズ
require_once("../bottle_assets/resizeimg.class.php");
$width = 400;
$height = 400;
		...
		if ($fix){
			...
			if ($line_link){
				...
				foreach ($line_link as $key => $value){
					list($temp_count,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,$image_0,) = split("&&",$value);
...
///////////////////////////////////
function write_data(){
	global $customer,$kana,$company,$section,$section2,$degree,$zip,$pref,$addr,$addr2,$phone,$mobile,$memo,$birthyear,$birthmonth,$birthday,$firsttime,$lasttime,$pic,$no_dm,$lastvisit,$dsp,$bottle_0,$bottle_num_0,$bottle_quant_0,$bottle_1,$bottle_num_1,$bottle_quant_1,$bottle_2,$bottle_num_2,$bottle_quant_2,$bottle_3,$bottle_num_3,$bottle_quant_3,$bottle_4,$bottle_num_4,$bottle_quant_4,$otherbottle,$otherbottle_num,$otherbottle_quant,$friends_0,$friends_1,$friends_2,$friends_3,$friends_4,$friends_5,$friends_6,$friends_7,$friends_8,$friends_9,$image_0;
	//カウンタ読み込み
...
//画像保存パス
$image_0 = $file_dir.$new_filename."_s.".$extension_s;
...
function delete_pic($radio){
	...
	foreach ($line_link as $value){
		list($count,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,$image_0,) = split("&&",$value);
		if ($count == $radio){
			@unlink($image_0);
			return;
		}
...
//利用停止
if ($ustop){
		$message_error = "現在100件以上の新規登録ができません";
		$message_error2 = "<br /><span>ご契約プランのご利用再開は<a href='/usces-member' target='_blank'>契約ユーザーログイン</a>からお手続きください</span>";
		if ($counts >= 100){
			$form_none = 1;
			$disabled = "disabled";
		}
...
if ($image_logs <> ""){ //画像登録オンオフ
//-------------------------------------------------------↓
print <<< END_OF_HTML
<tr>
    <th class="detail_tbl_t">画像登録 <div class="infotip"><abbr title="画像を登録します。" rel="tooltip">?</abbr></div></th>
    <td class="detail_tbl_t">
END_OF_HTML;

if (@$delete_sw){
	if ($image_0){
		echo "&nbsp;<img src='".$image_0."'>"."\n";
	}else{
		echo "&nbsp;";
	}
}else{
	if ($fix && $image_0){
		echo "&nbsp;<img src='".$image_0."'>"."\n";
		echo "	<br><input type='checkbox' name='delete_pic' value='delete'>↑バナー画像を削除<br>"."\n";
	}
	echo "	<input name='image_0' class='image_upload' type='file' $disabled>"."\n";
}
print <<< END_OF_HTML
</td>
  </tr>
...
<input type="button" value="ボトル入力項目追加" class="btn btn-success btn-sm bottles_add" title="入力項目追加" />
...
print <<< END_OF_HTML
  <tr>
    <th class="detail_tbl_t">友人知人情報 <div class="infotip"><abbr title="友人知人情報に入力した内容も検索時にヒットします。" rel="tooltip">?</abbr></div></th>
    <td class="detail_tbl_t"><input type="text" name="friends_0" id="friends_0" value="$friends_0" maxlength="40" class="form-control friends_form" placeholder="友人知人名" style="ime-mode: active;" $disabled>
    <input type="text" name="friends_1" id="friends_1" value="$friends_1" maxlength="40" class="form-control friends_form" placeholder="友人知人名" style="ime-mode: active;" $disabled>
    <input type="text" name="friends_2" id="friends_2" value="$friends_2" maxlength="40" class="form-control friends_form" placeholder="友人知人名" style="ime-mode: active;" $disabled>
    <input type="text" name="friends_3" id="friends_3" value="$friends_3" maxlength="40" class="form-control friends_form" placeholder="友人知人名" style="ime-mode: active;" $disabled>
    <input type="text" name="friends_4" id="friends_4" value="$friends_4" maxlength="40" class="form-control friends_form" placeholder="友人知人名" style="ime-mode: active;" $disabled>
    <input type="text" name="friends_5" id="friends_5" value="$friends_5" maxlength="40" class="form-control friends_form" placeholder="友人知人名" style="ime-mode: active;" $disabled>
<div class="clear"></div>
</td>
	</tr>
END_OF_HTML;
...
//-------------------------------------------------------↑
//	echo "<tr>"."\n";
//	echo "<th class='detail_tbl_t'>非表示</th>"."\n";
