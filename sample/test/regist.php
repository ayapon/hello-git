...
if ($submit){
	...
	if (!$kana){$message_error = "【ふりがな】は必須項目です";}
		...
		if (!@$reload && $upfile){
			...
			if ($fix && !$message_error){delete_img($delete);}
		}
		//エラーなし＆＆画像削除が選択
		if (!$message_error && $delete_img && $delete){
			delete_img($delete);//画像削除
		}
...
if (!$cnt){
	if ($counts >= $uplan_limit - 5 ){
		$remaining = ($uplan_limit - $counts);
		$message_error = "ご利用プランの登録可能件数はあと".$remaining."件です";
		$message_error2 = "<br /><span>プランのご変更は<a href='/usces-member' target='_blank'>契約ユーザーログイン</a>で行えます</span>";
	}
	if ($counts >= $uplan_limit){
		$message_error = "ご利用プランの登録可能件数は".$uplan_limit."件までです";
		$message_error2 = "<br /><span>プランのご変更は<a href='/usces-member' target='_blank'>契約ユーザーログイン</a>で行えます</span>";
		$form_none = 1;
		$disabled = "disabled";
	}
...
//-------------------------------------------------------↑
		if ($fname){
			echo "<div class='up_img_regist'><a href='".$fname."' class='fancybox'><img src='".$fname."' style='width:100px;height:auto;'></a></div>"."\n";
			echo "<label><input type='checkbox' name='fname' value='".$fname."' checked> チェックを外すとこの画像を削除</label><br />"."\n";
//			echo "<label><input type='checkbox' name='delete_img' value='delete'> この画像を削除</label><br />"."\n";
//			echo "<input type='hidden' name='fname' value='".$fname."'>"."\n";
		}
		echo "	<div class='upfile_regist'><label for='file_img'>＋画像を選択<input name='upfile' type='file' id='file_img' style='display:none;'></label></div>"."\n";
//-------------------------------------------------------↓
print <<< END_OF_HTML
	</td>
  </tr>
...
<input type="text" name="otherbottle" value="$otherbottle" class="form-category form-control otherbottle" maxlength="40" placeholder="その他のボトル" $disabled>
		<input type="text" name="otherbottle_quant" value="$otherbottle_quant" class="form-category3 form-control otherbottle" placeholder="タグ番号等" maxlength="4" $disabled><input type="text" name="otherbottle_quant" value="$otherbottle_quant" class="form-category2 form-control otherbottle" placeholder="残量等" maxlength="4" $disabled>
      </td>
</tr>
END_OF_HTML;
}
